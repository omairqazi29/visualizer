...
- **SupplyCalculator**: Waterfall = EB140k + FB_spill + EB45_spill + freeze_savings. Two data-driven corrections (Jun 2026): (1) EB-4/5 spillover uses total usage from DHS Yearbook CSV (not DOS consular-only — AOS unaffected by bans per Dorcas). (2) India EB-1 = total_eb1 − non_India_demand, where non-India demand is from live I-485 inventory (fallback: DHS Yearbook avg FY2023-2024).

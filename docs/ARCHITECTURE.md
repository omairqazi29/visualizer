...
- Backlog-ahead composition (`/api/predict`): `inventory_ahead` is **observed** — the USCIS I-485 inventory breaks out pending cases by priority-date year, and for India EB-1 every pending case has a PD <= 2023 (nobody past the 01DEC23 DOF can file yet). `pipeline_counted_ahead` is **modeled**: the I-140 "Approved Petitions Awaiting Visa Availability" report is country x category only with no priority-date dimension, so it is spread linearly from the live DOF cutoff to today. Optional `net_pipeline_overlap` removes the I-485 "Awaiting Availability" population, which both reports appear to count.

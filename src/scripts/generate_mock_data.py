...
def generate_dos_data(directory="data/DOS"):
    ...
    countries = ["India", "China - mainland born", "Mexico", "Philippines", "Dominican Republic", "Vietnam", "UK", "Canada"]
    ...
    for month in months:
        ...
        file_path = os.path.join(directory, f"{month} - IV Issuances by FSC or Place of Birth and Visa Class.xlsx")
...
def generate_inventory_data(file_path="data/eb_inventory_january_2026.xlsx"):
    ...
    df_india.to_excel(writer, sheet_name='India (EB1 EW3 EB4 CRW EB5)', startrow=3, index=False)
    
    writer.close()
    print(f"Generated {file_path}")

def generate_pipeline_data(file_path="data/eb_i140_i360_i526_performance_data_fy2025_q4_v1.xlsx"):
